...
train_path = "mnist/train/"  # 注意：和LeNet5_train.py 中使用的数据不一样
test_path = "mnist/test/"

#读取图片及其标签函数
def read_image(path):
    label_dir = [path+x for x in os.listdir(path) if os.path.isdir(path+x)] #os.listdir()方法用于返回指定的文件夹包含的文件或文件夹的名字的列表
    images = []                                                             #os.path.isdir()函数判断某一路径是否为目录
    labels = []
    for index,folder in enumerate(label_dir):
        for img in glob.glob(folder+'/*.png'):
            #print("reading the image:%s"%img) #我给注释掉了
            image = io.imread(img)
            image = transform.resize(image,(w,h,c)) #将所有的图片重新设置尺寸为32*32
            images.append(image)
            labels.append(index)
